...
longpoll = VkLongPoll(authorize,
                         group_id="216563568")  # отправляем запрос на сервер с помощью технологии long polling
print("Бот запущен!")


def menu(reseived_message, user_id):
    keyboard = VkKeyboard()  # добавить клавиатуру


    if reseived_message == "начать":
        keyboard.add_button("Текущая")
        keyboard.add_line()
        keyboard.add_button("6 часов")
...
for event in longpoll.listen():  # ждем от сервера ответа о произошедшем событии
    if event.type == VkEventType.MESSAGE_NEW and event.from_chat:
        ...
        print('юзер ид', user_id)
        reseived_message = event.text  # то сохраняем полученное сообщение
        reseived_message = reseived_message.translate(
            {ord(c): None for c in string.whitespace})  # если было введено раздельно, убрали пробелы

        chat = event.chat_id  # сохраняем номер чата
        print('из чата', chat)
        menu(reseived_message.lower(), user_id)
